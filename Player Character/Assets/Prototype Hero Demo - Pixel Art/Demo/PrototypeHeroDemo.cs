...
public class PrototypeHeroDemo : MonoBehaviour {
    ...
    //Allowances custom
    [SerializeField] bool       allow_double=true;
    [SerializeField] bool       allow_triple=true;
    [SerializeField] bool       allow_crouch=true;
    [Header("Effects")]
    ...
    void Update ()
    {
        //Disabling/enabling double, triple jump
        if(!allow_double)
        {
            m_double = true;
        }
        ...
        //Set Crouch in animator
        m_animator.SetBool("Crouch_t",m_crouch);
        ...
        //New properties
        //Crouch
        if (Input.GetKey("down") && m_grounded && m_disableMovementTimer < 0.0f)
        {
            m_animator.SetTrigger("Crouch");
            m_crouch = true;
        }

        if (!Input.GetKey("down") && m_grounded && m_disableMovementTimer < 0.0f && m_crouch)
        {
            m_animator.SetTrigger("Crouch");
            m_crouch = false;
        }
        ...
        // double jump code
        else if (Input.GetButtonDown("Jump") && !m_grounded && m_disableMovementTimer < 0.0f && !m_double)
        {
            m_animator.SetTrigger("Jump");
            m_grounded = false;
            m_animator.SetBool("Grounded", m_grounded);
        ...
        // triple jump code
        else if(Input.GetButtonDown("Jump") && !m_grounded && m_disableMovementTimer < 0.0f && m_double && !m_triple)
        {
            m_animator.SetTrigger("Jump");
            m_grounded = false;
            m_animator.SetBool("Grounded", m_grounded);
